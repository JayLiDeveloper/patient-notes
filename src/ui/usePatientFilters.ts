...
export function usePatientFilters(patients: Patient[]) {
  ...
  const filteredPatients = useMemo(
    () =>
      patients.filter((p) => {
        ...
        const matchesGender = patientGenderFilter
          ? p.gender === patientGenderFilter
          : true;
        let matchesDOB = true;
        if (dobFrom) {
          matchesDOB =
            matchesDOB && new Date(p.date_of_birth) >= new Date(dobFrom);
        }
        if (dobTo) {
          matchesDOB =
            matchesDOB && new Date(p.date_of_birth) <= new Date(dobTo);
        }
  ...
  const totalPatientPages = useMemo(
  ...
  const allGenders = useMemo(
    () => Array.from(new Set(patients.map((p) => p.gender))),
    [patients]
  );
